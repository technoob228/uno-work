...
Hermes как шестой встроенный харнесс, серверный headless-браузер для
bridge-команд и пачка фиксов preview-панели и ACP-рантайма.

> Функция ассистента (manager agent, Telegram-коннектор, раздел Assistants)
> в этот релиз сознательно не вошла — она остаётся в `main` и выйдет отдельно.

### Hermes — шестой встроенный харнесс

- `HermesDriver` + `HermesAdapter`: hermes acp как полноценный провайдер
  (spawn, session/new, session/load, набор моделей).
- `HermesSettings` в contracts: схема провайдера и дефолтная модель.
- Hermes в provider picker, настройках и иконках веба.
- Фикс: MCP-тулсеты Hermes переживают `set_model` — per-thread `HERMES_HOME`.
- Фикс: подавлен warning `thinking_tokens`, reasoning разрешён для Fable 5.

### Серверный headless-браузер

Bridge-команды (`/api/browser/command`) теперь умеет исполнять не только
десктопный webview, но и headless-браузер на сервере (playwright-core):
если живого клиента-подписчика нет, команда уходит серверному исполнителю
со scoped bridge-контекстом. Это открывает сценарии «агент управляет
браузером» без открытого десктопа.

### Фиксы

- Browser-вкладки привязаны к своему проекту, а не к тому, что на экране.
- Preview-панель скрывается на settings-маршрутах.
- Активное окружение не сбрасывается на re-emit'ах welcome/serverConfig.
- ACP: replay `session/load` больше не ре-эмитится в проекцию.
- ACP: уникальные id сегментов на каждый запуск рантайма — резюмнутая
  сессия больше не теряет новые ответы из-за коллизии message id.
- Из пикеров убраны coming-soon провайдеры.
