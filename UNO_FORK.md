...
### 9. Inbox-секции в сайдбаре и snooze (ручной порт из апстрима)

Внутри каждой группы проекта чаты раскладываются на Pinned → Active → Snoozed (свёрнутая полка «Snoozed · N») → Done (settled: тише, по времени, первые 5 + «Show N more»). Флаг Labs `inboxSections` (по умолчанию ON); выключен — старый плоский список. Старый флаг `sidebarInbox` переименован в `allMachinesSidebar` (значение читается из старого ключа и переезжает при следующей записи, `featureFlags.ts`).

- **Сервер/контракты (совместимо с апстримом по проводу):** `snoozedUntil`/`snoozedAt` на `OrchestrationThread`/`OrchestrationThreadShell` (optional), команды `thread.snooze {snoozedUntil}` / `thread.unsnooze {reason:"user"}`, события `thread.snoozed` / `thread.unsnoozed {reason: user|activity}`, миграция `043_ProjectionThreadsSnoozed`, capability `threadSnooze` в дескрипторе (старые демоны → кнопки Snooze скрыты). Decider отказывает при открытом approval/вопросе и при только что отправленном, ещё не подхваченном сообщении; сам снимает snooze (`reason: activity`) на `thread.turn.start` и на `approval.requested`/`user-input.requested`. Истечение `snoozedUntil` — только на чтении, без таймера.
- **Web:** правила секций — `apps/web/src/components/Sidebar.sections.ts`, snooze-правила/пресеты — `Sidebar.snooze.ts`. Settled считается на клиенте (у апстрима — серверный `settledOverride`): последний ход завершён, ничего не бежит и не ждёт человека, простой > 3 дней (дефолт апстрима). Отличия от апстрима: pinned никогда не уходит в Done; чат, который ждёт человека, всегда виден (из полки Snoozed — наверх Active, pinned остаётся в Pinned). Состояние полок — session-only в `uiStateStore.sidebarShelfExpandedByProjectKey`. Часы — `hooks/useMinuteClock.ts`.
- **Не портировано:** drag между секциями (`Sidebar.drag/motion/pointer`) — у нас нет серверного порядка (`pinOrderKey`/`activeOrderKey`) и треды не в sortable-списке; ручной settle/unsettle; «Woke»-индикатор; compact thread list mode.
