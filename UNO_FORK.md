...
- **Безопасная версия (0.0.57, решение 15.09, план `.plans/22-workspace-and-safe-continue.md` п.1).** До 0.0.56 снимок пушился `--force --no-verify` в ветку `uno/continue/<threadId>` на `origin`, `.env` копировался по умолчанию, а на цели файлы ложились поверх текущей ветки. Теперь: **ничего не пушится**, код идёт через клиент; `.env` — только по галке (по умолчанию выкл.); на цели — отдельный worktree на новой ветке, рабочая копия пользователя не трогается.
- **Контракты:** `packages/contracts/src/threadContinue.ts`. RPC: `thread.continue.inspect` (цель, read-only: есть ли папка, git ли это), `thread.continue.snapshot` (источник: снимок + bundle, возвращает `transferId`, размер, SHA-256, число чанков, seed, настройки треда, `remoteUrl` только для клона, `envText` только при `includeEnv: true`), `thread.continue.readChunk` (источник) / `thread.continue.writeChunk` (цель) — base64-чанки по 2 МиБ (`THREAD_CONTINUE_CHUNK_BYTES`), лимит bundle 1 ГиБ, `thread.continue.land` (цель), `thread.continue.discard` (любая сторона, best-effort), `thread.continue.complete` (источник). `ThreadContinueError` с machine-readable `reason` (новые: `too_large`, `transfer_not_found`, `transfer_incomplete`, `base_missing`, `worktree_failed`).
- **Версии и совместимость.** Демон с новым протоколом объявляет `capabilities.threadContinueDirect` в дескрипторе окружения. Клиент не стартует, если флага нет у источника или цели (строка «Update Uno Work on <machine>…», у машины в списке — «Needs update»; для неподключённой цели проверка повторяется после коннекта, в шаге connecting). Старые RPC `thread.continue.prepare/receive/cleanup` оставлены в группе только ради совместимости: новый демон всегда отвечает `invalid_request` с текстом `THREAD_CONTINUE_LEGACY_CLIENT_MESSAGE` («обновите приложение»), так что старый клиент (0.0.53–0.0.56) через новый демон ничего не опубликует. Старый демон + новый клиент: клиент видит отсутствие флага и не зовёт методы.
- **Файлы: git bundle через клиент.** `apps/server/src/git/continueTransport.ts`: снимок рабочего дерева (tracked + untracked, без ignored) чекпоинт-машинерией (`CheckpointStore.captureCheckpoint` с `parents`/`message`) в скрытый ref `refs/t3/continue/<transferId>` с родителем HEAD → `git bundle create <file> <ref> --not --remotes` (в bundle только коммиты, которых нет ни на одной remote-tracking ветке источника, + снимок; без remote — вся история) → ref удаляется. Файлы bundle — `apps/server/src/git/continueTransferStore.ts`: `<baseDir>/tmp/continue/{outgoing,incoming}/<transferId>.bundle`, transferId только UUID, запись чанка по offset (повтор чанка обрезает файл, дырка → `transfer_incomplete`), подчистка старше 24 ч при каждом snapshot. Клиент (`apps/web/src/continueOnMachine.ts`) читает чанки с источника и пишет на цель.
- **Цель (`land`).** Проверка размера и SHA-256 → проект: `existing` (зарегистрирован по пути) или `create` (клон по `remoteUrl` — это чтение; без remote — `git init` + вся история из bundle; уже существующий git-клон усыновляется) → модель (до любых git-записей, чтобы отказ не оставлял веток) → `git bundle verify`; если нет базового коммита — `git fetch --all` из remotes цели и повторная проверка, иначе `base_missing` («Pull the latest commits…») → `git fetch <bundle>` в скрытый ref, сверка oid → `git worktree add -b uno/continue/<первые 8 символов id треда>[-N] <baseDir>/worktrees/<repo>/<branch> <baseCommit>` → `restoreCheckpoint` снимка в worktree (изменения остаются незакоммиченными, как на источнике; у unborn-источника ветка стартует прямо со снимка) → удаление ref и входящего bundle → `.env` (если прислан) пишется в новый worktree, не в корень проекта → `thread.create` с `branch`/`worktreePath`, seed, активность `thread.continued.from` (branch, worktreePath, commit, baseCommit).
...
- **Обработчики:** `apps/server/src/orchestration/continueOnMachine.ts` (явные deps, как `plugins/panelThread.ts`), провязка в `ws.ts` (`transfers` = `makeContinueTransferStore({ rootDir: <tempDir>/continue })`, `worktreesDir` из `ServerConfig`). `.env` на источнике: сначала из worktree треда, иначе из корня проекта. Модель — та же, если харнесс установлен, иначе default проекта / auto-bootstrap с пометкой в seed. Origin команд `{kind:"system", component:"thread-continue"}`.
- **Web:** степ-машина connecting (коннект + проверка `threadContinueDirect`) → saving (snapshot) → sending (чанки, прогресс «X MB of Y MB») → preparing (land) → marking (complete) → opening; discard на источнике после land — молча. Чекпоинт прогресса: retry продолжает с первого недошедшего чанка; `transfer_not_found` на источнике сбрасывает снимок (retry снимет новый), `transfer_incomplete` на цели — один автоматический рестарт загрузки / повторная отправка при retry. `ContinueOnMachineDialog.tsx`: у каждой машины «Has this project» / «Project will be added» (машины с проектом сверху) или «Needs update»; под списком — что будет на цели («opens in a separate copy of the project on a new branch. Your files … stay as they are»), подтверждения «Replace them» больше нет; галка «Also copy .env secrets» выключена по умолчанию, с пояснением. Строки — `continueOnMachineCopy.ts`.
- **Тесты:** `handoff.test.ts`; `orchestration/continueOnMachine.test.ts` (fake transport/transfers: снимок без push, `.env` только по флагу и из worktree, too_large, land в worktree на новой ветке, нумерация `-N`, fetch недостающей базы, `base_missing`, `commit_mismatch`, `no_model` до git-записей, clone / `git init` без remote, legacy-отказ); `git/continueTransport.test.ts` (реальный git: неотправленный локальный коммит + правка + новый файл + удаление + ignored едут bundle'ом в новый worktree, рабочая копия цели и её незакоммиченные файлы не тронуты, bare-«GitHub» не изменился; догрузка базы из remote цели; репозиторий без remote → свежий `git init`); `git/continueTransferStore.test.ts` (чанки туда-обратно, повтор/дырка, UUID-валидация, TTL); `apps/web/src/continueOnMachine.test.ts` (порядок шагов, `.env` только с галкой, стоп при старой версии, resume с чанка, рестарт загрузки, гейтинг кнопки). Живая проверка двумя стендовыми демонами — см. отчёт по ветке `feat/safe-continue`.
- **Компромиссы:** worktree на цели не удаляется автоматически (как и обычные worktree тредов); ветка `uno/continue/*` на цели — локальная. Список целей — только сохранённые окружения (как раньше), локальный primary-демон целью не предлагается. Если `land` упал после `git worktree add` (например, restore), worktree остаётся — retry возьмёт следующее имя `-N`.

### 9. Сайдбар: список чатов (порт Sidebar v2 из апстрима), inbox-секции, snooze, settle

**По умолчанию — плоский список чатов** (`apps/web/src/components/Sidebar.tsx`), порт апстримного Sidebar v2: #4026 (плоский список), #5672 (v2 по умолчанию, старый → `LegacySidebar.tsx`), #11315 (одна строка шапки: поиск + scope проекта + новый проект + новый чат), #11595 (полки Snoozed/Settled прижаты к низу), полировка карточек из #4274/#5909/#10569/#10713/#11466. Старый сайдбар с группировкой по проектам — `LegacySidebar.tsx` (чистый rename, как у апстрима), включается Labs-флагом `legacySidebar` («Sidebar (legacy)», по умолчанию OFF), выбор — в `AppSidebarLayout.tsx`.

- **Разметка:** Pinned + Active — карточки (проект / заголовок / ветка + PR + терминал + чип машины + иконка провайдера), статус справа в первой строке (Working с таймером / Approval / Input / Failed / Done), на hover — часы Snooze и «✓ Settle». Snoozed и Settled — тонкие строки в свёрнутых полках («Snoozed (N)», «Settled (N)»), settled листается страницами 10 + 25 («Show N more»), открытый чат всегда виден даже в свёрнутой полке. Шапка — `sidebar/SidebarThreadHeader.tsx` (почти дословно апстрим), scope-комбобокс «All projects» / «Helper» / проекты, выбранный проект показывает свою favicon вместо папки; scope хранится в localStorage (`uno-work:sidebar:project-scope`). Кнопка «New chat»: в scope проекта — сразу в нём; иначе один проект или Shift+click — в текущем, несколько — палитра «New chat in…» (новый intent `new-thread-in` в `commandPaletteStore`). Токены строк (`--sidebar-row-hover/active/selected`, `--secondary-label`, `--sidebar-content-inset`) названы как у апстрима — классы в разметке совпадают.
- **Хром:** `sidebar/SidebarChrome.tsx` — наша шапка с логотипом «U Work» (без T3-артов) и футер как у апстрима — строка иконок (Settings, Helper) + компактный свитчер машины (`SidebarEnvSwitcher`/`SidebarWorkspaceSwitcher` получили `variant="compact"`; legacy использует `card`).
- **Фичи форка в новом сайдбаре:** данные из zustand-стора с учётом machine scope (Labs `allMachinesSidebar`); чип машины + облачко offline на чужих машинах (`sidebar/useSidebarMachineIdentities.ts`); бот-иконка на чатах, созданных агентом; Helper-чаты (assistant-проекты) не засоряют общий список — они за scope «Helper» и иконкой в футере (открытый Helper-чат виден); контекстное меню: Rename, Pin/Unpin, Settle/Un-settle, Snooze/Wake (+ «Pick time…»), Mark unread, Copy Path, Copy chat ID, Continue on another machine, Archive, Delete; мультиселект (⌘/Shift+click) с Settle/Snooze/Mark unread/Delete; ⌘1…9 и prev/next по порядку списка; поиск по заголовку со стрелками/Enter/Esc; двойной клик — переименование; prewarm подписок первых чатов.
- **Не портировано:** drag между секциями и ручной порядок (у нас нет `pinOrderKey`/`activeOrderKey`), блок черновиков в сайдбаре, «Woke»-индикатор, compact rail (апстрим откатил в #11685), drop файлов на строку, ссылки на несколько PR / стеки PR, Pull Requests и Usage в футере, project settings из scope-меню, «monitoring»-статус, серверный auto-settle reactor.

**Inbox-секции** (`Sidebar.sections.ts`, общие для обоих сайдбаров): Pinned → Active → Snoozed → Settled. В legacy — внутри каждой группы проекта под флагом `inboxSections` (по умолчанию ON, «Done · N»). Старый флаг `sidebarInbox` переименован в `allMachinesSidebar` (значение читается из старого ключа и переезжает при следующей записи, `featureFlags.ts`).

- **Snooze (сервер, совместимо с апстримом по проводу):** `snoozedUntil`/`snoozedAt` на `OrchestrationThread`/`OrchestrationThreadShell` (optional), команды `thread.snooze {snoozedUntil}` / `thread.unsnooze {reason:"user"}`, события `thread.snoozed` / `thread.unsnoozed {reason: user|activity}`, миграция `043_ProjectionThreadsSnoozed`, capability `threadSnooze`. Decider отказывает при открытом approval/вопросе и при только что отправленном, ещё не подхваченном сообщении; сам снимает snooze (`reason: activity`) на `thread.turn.start` и на `approval.requested`/`user-input.requested`. Истечение `snoozedUntil` — только на чтении.
- **Settle (сервер, совместимо с апстримом по проводу):** `settledOverride` (`"settled"|"active"|null`) + `settledAt` на треде и шелле, команды `thread.settle` / `thread.unsettle {reason:"user"}`, события `thread.settled` / `thread.unsettled {reason: user|activity}`, миграция `045_ProjectionThreadsSettled`, capability `threadSettlement` (старые демоны → Settle/Un-settle скрыты). Settle отклоняется, пока сессия работает, открыт approval/вопрос или висит неподхваченное сообщение; снимает pin и snooze. Новый ход, оживающая сессия (`starting/running`) и approval/input-запрос сбрасывают override в `null` (`reason: activity`). **Автоматический settle остаётся клиентским**: override `null` + последний ход завершён, ничего не ждёт, простой > 3 дней (дефолт апстрима). `"active"` (после Un-settle) выводит чат из авто-правила до следующей активности. Серверного `ThreadSettlementReactor`/`sidebarAutoSettleAfterDays` у нас нет — это сознательно: авто-правило без таймера на сервере, как было.
- **Web:** правила секций — `Sidebar.sections.ts` (explicit settle обгоняет pin; чат, который ждёт человека, всегда виден; settled сортируется по `settledAt`, затем по последней активности), snooze-правила/пресеты — `Sidebar.snooze.ts`, capability-хуки — `environments/threadSnoozeSupport.ts`, `environments/threadSettlementSupport.ts`. Полки legacy — session-only в `uiStateStore.sidebarShelfExpandedByProjectKey`; полки нового сайдбара — localStorage (`uno-work:sidebar:{settled,snoozed}-expanded`).
...
- `apps/web/src/components/Sidebar.tsx` / `LegacySidebar.tsx` — наш `Sidebar.tsx` написан заново по структуре апстримного v2 на данных нашего стора (у апстрима client-runtime атомы), при мердже переносить правки поштучно, не резолвить построчно; `LegacySidebar.tsx` у нас и у апстрима — разные файлы (наш — бывший форковый `Sidebar.tsx`). `Sidebar.logic.ts` — функции v2 под апстримными именами, внизу файла. `sidebar/SidebarThreadHeader.tsx` — почти дословная копия.
- `apps/server/src/orchestration/decider.ts`, `packages/contracts/src/orchestration.ts` — settle/snooze порт; миграции 043/045 у нас, у апстрима 033/034.
