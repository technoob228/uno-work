 ...
 * следит за ними и дописывает, пока человек не взял управление.
 */
export function buildAgentThreadsInstructions(): string {
  ...
  return `## Другие треды: запустить и вести
...
Тебе видны только треды, созданные этим чатом; остальные — 404. Ответ 403 \`thread_context_required\` значит, что твой харнесс не привязан к конкретному треду и вести треды отсюда нельзя — скажи об этом пользователю, не повторяй попытки. Не добавляй \`-f\` к curl: причина отказа приходит в теле ответа (\`{"ok":false,"error":"…","message":"…"}\`). Не плоди треды без нужды: один тред — одна самостоятельная подзадача.`;
}
