...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest()))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      live("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
    ...
    Layer.provideMerge(UnoGatewayKeyTest()),
      ...
      live("includes unavailable instance snapshots in getProviders", () =>
        Effect.gen(function* () {
          const serverSettings = yield* makeMutableServerSettingsService(
            Schema.decodeSync(ServerSettings)(
              deepMerge(DEFAULT_SERVER_SETTINGS, {
    ...
    Layer.provideMerge(UnoGatewayKeyTest()),
      ...
      live("keeps cursor disabled and skips probing when the provider setting is disabled", () =>
        Effect.gen(function* () {
          const serverSettings = yield* makeMutableServerSettingsService(
            Schema.decodeSync(ServerSettings)(
              deepMerge(DEFAULT_SERVER_SETTINGS, {
    ...
    Layer.provideMerge(UnoGatewayKeyTest()),
          ...
          yield* Effect.gen(function* () {
            ...
            assert.strictEqual(cursorProvider?.message, "Cursor is disabled in Uno Work settings.");
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Uno Work settings.");
        }),
      );
    });
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("includes Claude Sonnet 5 and Opus 5 on supported Claude Code versions", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
      ...
      it.effect("hides Claude Opus 5 until Claude Code 2.1.219", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
      ...
      it.effect("hides Claude Sonnet 5 until Claude Code 2.1.197", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
