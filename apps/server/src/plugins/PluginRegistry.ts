...
export interface PluginRegistryLiveOptions {
  /**
   * Интервал страховочного свипа. `fs.watch` не даёт гарантий доставки
   * (события в окно перевески вотчеров и до фактического arming форкнутого
   * стрима теряются ОС безвозвратно) — периодический reload с дедупликацией
   * по сигнатуре превращает потерянное событие в задержку ≤ интервала.
   */
  ...
  Effect.gen(function* () {
    ...
    const attachWatchers = (directories: ReadonlyArray<string>) =>
      Effect.gen(function* () {
        ...
        // Всё, что случилось в окно «старые вотчеры сняты — новые ещё не armed»,
        // потеряно ОС. Само-сигнал гоняет ещё один reload после перевески: он
        // дёшев (сигнатурная дедупликация) и сходится — набор директорий уже
        // актуален, второй перевески не будет.
        yield* PubSub.publish(watchSignals, undefined);
      });
    ...
    const startWatcher = Effect.gen(function* () {
      ...
      // Страховочный свип: fs.watch может потерять событие до arming форкнутого
      // стрима — периодический reload делает потерю задержкой, а не зависанием.
      const sweepIntervalMs = Math.max(200, options?.sweepIntervalMs ?? DEFAULT_SWEEP_INTERVAL_MS);
      yield* reloadSafely.pipe(
