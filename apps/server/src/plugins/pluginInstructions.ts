...
**panel** — статическая «софтинка» плагина во вкладке правой панели приложения: \`"panel": { "title": "Deploys", "path": "panel/index.html" }\`. \`path\` — файл внутри директории плагина (относительный, без \`..\`; должен существовать, иначе плагин помечается невалидным). Необязательное поле \`chat\` добавляет рядом с панелью чат приложения — см. «кастомный AI-интерфейс» ниже. Панель открывается из Settings → Extensions и из меню «+» в табстрипе правой панели.
...
**panel.chat — кастомный AI-интерфейс.** Если пользователь хочет «свой экран для работы с агентом» (а не просто дашборд), добавь в \`panel\` секцию \`chat\`: рядом с панелью появится настоящий чат приложения, привязанный к одному треду.
...
\`\`\`json
{
  ...
  "panel": {
    ...
    "chat": { "threadTag": "reports", "visibility": "answers-only" }
  }
}
\`\`\`

- \`threadTag\` (обязателен) — тот же тег, что у \`sendToThread\`. Кнопки панели и чат попадают в ОДИН тред: панель нажимает \`uno.sendToThread("...", "reports")\`, ответ пользователь читает в чате рядом и может продолжить диалог руками.
- \`visibility\` (по желанию, по умолчанию \`"full"\`):
  - \`"full"\` — обычный таймлайн;
  - \`"answers-only"\` — только ответы: реплики пользователя, готовые ответы ассистента и планы; команды, чтения файлов и прочий инструментальный шум скрыты;
  - \`"composer-only"\` — только поле ввода (панель сама рисует результат, например читая \`data.json\`).
- Approvals и уточняющие вопросы агента показываются всегда — они живут в поле ввода, иначе работу нельзя было бы подтвердить.
- Права треда наследуются и в чате не переключаются: плагин может права только сузить, но не расширить.
- Вкладка делится на две части (панель сверху, чат снизу), границу можно двигать. Кнопка «фокус» в правой панели растягивает всё это на весь экран — это и есть «кастомный интерфейс на весь экран».

Рецепт целиком: директория-плагин → \`panel.path\` (HTML+inline JS) + \`panel.chat.threadTag\` → внутри страницы сниппет моста → кнопки зовут \`uno.sendToThread(text, "<тот же threadTag>")\` → \`uno.subscribe("thread.*", …)\` перерисовывает панель, когда агент закончил ход → cron (если нужны фоновые данные) пишет \`data.json\` рядом. Так пользователь живёт в правой панели: сверху твой интерфейс, снизу диалог с агентом.
