...
import { listAssistantLlmModels, pushAccountDefaultAi } from "../../lib/assistantLlmApi";
...
export function AssistantEngineBanner(props: { readonly environmentId: EnvironmentId }) {
  ...
  // Chats Uno starts run on the account's default AI; the daemon of a Work
  // box can't read the account itself, so hand it what this client sees.
  useEffect(() => {
    if (!llm.supported || defaultAi === null) return;
    void pushAccountDefaultAi({ environmentId: props.environmentId, value: defaultAi }).catch(
      () => undefined,
    );
  }, [defaultAi, llm.supported, props.environmentId]);
