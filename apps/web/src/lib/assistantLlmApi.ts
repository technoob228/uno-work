...
/** Tell the daemon the account's default AI (it may only hold a gateway key). */
export function pushAccountDefaultAi(
  input: EnvironmentScoped & { readonly value: string | null },
): Promise<{ readonly ok: true }> {
  return environmentFetchJson({
    environmentId: input.environmentId,
    pathname: "/api/manager/assistant/default-ai",
    method: "POST",
